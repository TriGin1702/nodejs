...
router1.post("/", (req, res) => {
  ...
  axios
    .post(`${process.env.DOMAIN}:${process.env.PORT}/${process.env.API_ACC}/register`, customer)
    .then((response) => {
    ...
    .catch((error) => {
      if (error.response && error.response.status === 400) {
        ...
        return res.status(400).send('<script>alert("' + error.response.data + '"); window.location.href = window.location.href;</script>');
      } else {
        console.error(error);
