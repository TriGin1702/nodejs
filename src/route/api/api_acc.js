...
router2.post("/register", async(req, res) => {
    try {
        ...
        const existingUser = await new Promise((resolve, reject) => {
            connect.query(
                `SELECT * FROM customer WHERE account = ?`, [accountName, password],
                (err, result) => {
                    if (err) {
                        console.error(err);
                        reject(err);
        ...
        if (existingUser.length > 0) {
            ...
            connect.query(
                `INSERT INTO customer (name, gender, age, account, password) VALUES (?, ?, ?, ?, ?)`, [name, gender, age, accountName, password],
                (err, result) => {
                    if (err) {
                        console.error(err);
                        res.status(500).send("Error");
...
router2.post("/", async(req, res) => {
    try {
        ...
        const acc = await new Promise((resolve, reject) => {
            connect.query(
                `SELECT * FROM customer where account = ? and password = ? `, [accountName, password],
                (err, rows) => {
                    if (err) reject(err);
                    resolve(rows);
                }
