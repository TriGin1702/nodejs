...
router.post('/quantity', authenticateToken, async function(req, res) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            connect.query(
                sql, [checkedProducts, user_id, quantity],
                (error, results) => {
                    if (error) {
...
router.post('/address', authenticateToken, async(req, res) => {
    ...
    try {
        ...
        for (const product of checkedProducts) {
            await new Promise((resolve, reject) => {
                connect.query(
                    'CALL InsertOrUpdateAddressAndSetBillStatus(?, ?, ?, ?, ?, ?, ?)', [user_id, product, city, district, address, name, phoneNumber],
                    (error, results) => {
                        if (error) {
...
router.get('/:user_id', authenticateToken, async function(req, res) {
    try {
        ...
        const data_cart = await new Promise((resolve, reject) => {
            connect.query(
                'CALL GetCustomerCartDetails(?)', [user_id],
...
router.post('/:user_id', authenticateToken, async function(req, res) {
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            connect.query(
                'CALL AddProductToCart(?, ?)', [user_id, idProduct],
                (error, results) => {
                    if (error) {
