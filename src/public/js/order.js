...
function showPayment(event) {
  const selectedRowInput = event.target.closest("tr").querySelector('input[name="id_ad"]');
  if (!selectedRowInput) {
...
async function submitPayment() {
  ...
  console.log(firstName + " " + phoneNumber + " " + selectedCity + " " + selectedDistrict + " " + address + " " + id_ad);
  if (isValidAddress(address) && isValidPhoneNumber(phoneNumber) && isValidName(firstName)) {
    await axios
      .post("/order/address", {
        id_ad,
        firstName,
